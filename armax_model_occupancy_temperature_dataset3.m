...
zonetemp_testing = zonetemp(10001:end,:);
%load occupancy_testing2
N = 2; % Index of independent variable
y1 = zonetemp_training(:,1);
u1 = zonetemp_training(:,N);
u1_1 = zonetemp_training(:,4);
u1_2 = zonetemp_training(:,6);
u1_3 = zonetemp_training(:,2);
% u1_3 = occupancy_training(:,2);
% u1_4 = occupancy_training(:,5);
y2 = zonetemp_testing(:,1);
u2 = zonetemp_testing(:,N);
u2_1 = zonetemp_testing(:,4);
u2_2 = zonetemp_testing(:,6);
u2_3 = zonetemp_testing(:,2);
...
th = armax(data_training,'na',20,'nb',[20 20 20],'nc',20,'nk',[1 1 1]);
% th = [];
% for i = 1:30
...
%compare(data_testing,th2,th4)
% State-space model
% ms = n4sid(data_training, 3, 'InputDelay',1);
...
[y,fit,x0]=compare(data_testing,th);
dataFilt = idfilt(y,{[1/3 1/3 1/3],[1]});
ynew1 = dataFilt.y(3:end);
save('ynew1.mat','ynew1')