...
x5 = occupancy_training(:,5); % Occupancy
x5_nor = (x5-min(x5)*ones(size(x5,1),1))*(1/(max(x5)-min(x5)));
% A matrix to store the normalized mutual information
...
for i = 1:size(occupancy_training,2)
    for j = 1:size(occupancy_training,2)
        estimate = nmi(occupancy_training_p(:,i)',occupancy_training_p(:,j)'); % Call the normalized mutual information function
